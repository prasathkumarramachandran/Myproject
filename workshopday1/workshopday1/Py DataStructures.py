...
for i in range(20):
    print(w[len(w)-i-1])
 ##find which half of w has more z s
...
assign_txt.count("Modi")
"A" <"a"
"A"< "B"

#find number of capital letters using above comparisons and a for loop
...
lenl1= len(l1)
print (lenl1)
#incomplete loop complete the bubble sort
for j in range(lenl1):
    for i in range(lenl1-1):
        if l1[i]>= l1[i+1]:
            t=l1[i+1]
            l1[i+1]=l1[i]
            l1[i]=t
...
#Method 2
s2=[x**2 for x in c ] #Very concise way

### Lambdas, map, filter and reduce is now deprecated in python 3.0
# So, We use functools.reduce() to replicate the functionality of list comprehension
import functools

mass1=[45,55,65,76]
sqm = list(map(lambda x:x*2,mass1))
...
# for a list of strings return a list of length of strings


#filter function operates to subset the list to those satisfying the condition

bmi_f=list(filter(lambda x:x<20,bmi))


#reduce functions wraps all results inside out


print (bmi_f)
su=functools.reduce(lambda x,y:x+y,bmi)
print (su)




fact4=functools.reduce(lambda x,y:x*y,range(1,5))


fact5=functools.reduce(lambda x,y:x*y,range(1,6))
...
liString = ['abc','abd','a','cfgg','adefff','aadadaaaaaa']

#for a list of strings find the longest string by 
#forming a dictionary of strings and their lengths as values 

#traverse through the list and two starting variables counter and
#string    
...
# modules interlude
import os
os.chdir('C:\\aricent\\content\\day3\\ML Data Manipulation')
